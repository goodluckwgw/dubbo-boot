...
public class BookAdminService {
	...
	public void modifyBook(Book book, Long currentAccountId) {
		if (!currentAccountId.equals(book.owner.id)) {
			logger.error("user:" + currentAccountId
					+ " try to modified a book:" + book.id
					+ " which is not him");
			throw new ServiceException("User can't modify others book",
		...
		if (orginalBook == null) {
			logger.error("user:" + currentAccountId
					+ " try to modified a book:" + book.id
					+ " which is not exist");
			throw new ServiceException("The Book is not exist",
	...
	public void deleteBook(Long id, Long currentAccountId) {
		...
		if (book == null) {
			logger.error("user:" + currentAccountId + " try to delete a book:"
					+ id + " which is not exist");
			throw new ServiceException("The Book is not exist",
					ErrorCode.BAD_REQUEST);
		}
		...
		if (!currentAccountId.equals(book.owner.id)) {
			logger.error("user:" + currentAccountId + " try to delete a book:"
					+ book.id + " which is not him");
			throw new ServiceException("User can't delete others book",
					ErrorCode.BOOK_OWNERSHIP_WRONG);
		}
